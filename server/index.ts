...
if (!SARVAM_API_KEY) {
  console.warn('Warning: Missing Sarvam AI API key. Some features may not work.');
}

// Add status endpoint to check Sarvam AI availability
app.get('/api/stt-status', (_req, res) => {
    res.json({ 
      status: 'ready',
      sarvamApiKeyPresent: !!SARVAM_API_KEY,
      geminiApiKeyPresent: !!GEMINI_API_KEY,
      whisperReady: true
    });
});
...
// Initialize Google Generative AI with API version v1
...
async function convertAudioToWav(inputBuffer: Buffer, mimeType: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    fs.writeFileSync(inputPath, inputBuffer);
      ...
      console.error('Error during audio conversion setup:', err);
      ...
      reject(new Error(`Failed to setup audio conversion: ${err.message}`));
    }
  });
}
...
IMPORTANT: Do not include any markdown formatting or code block tags. Return only the JSON object.`;
    ...
    console.log('Sending request to Gemini with model: gemini-1.5-flash-002');
    console.log('Text length:', text.length, 'characters');
    console.log('First 100 characters of text:', text.substring(0, 100));

    // Split long text into chunks if needed
    const maxChunkSize = 30000;  // Gemini has a context window of ~30k tokens
    let analysisText = '';
    
    if (text.length > maxChunkSize) {
      // Process text in chunks
      const chunks = text.match(new RegExp(`.{1,${maxChunkSize}}`, 'g')) || [];
      for (const chunk of chunks) {
        const chunkResult = await model.generateContent([
          `Analyze this part of the conversation and return a JSON object with the same structure as before:\n${chunk}`
        ]);
        const chunkResponse = await chunkResult.response;
        analysisText += chunkResponse.text().trim();
      }
    } else {
...
app.post('/api/generate-questions', async (req, res) => {
  try {
    const { topic, interviewee: role, mode, context, numQuestions = 3 } = req.body;
    ...
    if (isNaN(parsedNumQuestions) || parsedNumQuestions < 1) {
      ...
      numQuestions = 3;
    } else if (parsedNumQuestions > 10) {
      console.warn(`numQuestions too high: ${parsedNumQuestions}, capping at 10`);
      numQuestions = 10;
    }
    ...
    const questions = await generateInterviewQuestions(topic, role, mode, parsedNumQuestions);
    ...
    if (generatedCount !== parsedNumQuestions) {
      console.warn(`Generated ${generatedCount} questions but expected ${parsedNumQuestions}`);
    }
...
app.post('/api/analyze-interview', async (req, res) => {
  try {
    ...
    const hasSubstantiveAnswers = req.body.answers.some(a => 
    ...
    if (!hasSubstantiveAnswers) {
      ...
      return res.json({
        ...
        question_by_question_feedback: req.body.answers.map(a => ({
          question: a.question,
          feedback: `Your response "${a.answer}" was too brief. A strong answer would include specific examples and details about your experience.`,
          strengths: [],
          areas_for_improvement: [
            "Provide a more detailed and substantive response",
            "Include specific examples from your experience",
            "Structure your answer with context, actions taken, and results achieved"
          ]
        })),
        recommendations: {
          key_action_items: [
...
app.post('/api/transcribe', upload.single('audio'), async (req, res) => {
  try {
    ...
    try {
      ...
      // Format the result
      const chunks = result.words.map((word: any) => ({
        text: word.text,
        start: word.start,
        end: word.end
      }));
